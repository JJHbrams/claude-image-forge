...
T3(SVG)로 내려가는 건 **이 기계에 이미지 모델이 아예 없을 때뿐**이다. 깔려 있는데 꺼져
있거나 설정이 어긋난 거라면 체인은 `tier: "blocked"` (exit 4)로 멈추고 **뭘 켜야 하는지**
알려준다. 그냥 SVG를 그려버리면 놀고 있는 GPU 옆에서 손으로 그라디언트를 칠하는 꼴이고,
사용자는 자기가 뭘 안 켰는지도 모른 채 못한 결과물을 받는다.

```json
{"ok":false,"tier":"blocked",
 "remedies":["[comfy] Start ComfyUI, then retry:\n  \"D:\\ai\\ComfyUI_windows_portable\\run_nvidia_gpu.bat\""]}
```

ComfyUI가 꺼져 있으면 드라이브를 2단계까지 훑어 설치 위치를 찾아 **실제 실행 명령**을
준다. 탐색이 닿지 않는 곳에 있으면 `.env` 의 `COMFY_DIR` 로 지정하면 된다.
